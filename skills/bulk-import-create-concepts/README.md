...
`reference/ocl-bulk-import.md` is the authority on how OCL itself behaves: the JSONL
line format, mandatory fields, every validation rule with its exact server message, the
lookup vocabularies, `__action` semantics, result buckets and upload limits. The skill
is self-contained — nothing here requires another repository or an external document.
...
| `concepts[]` | `id`, `external_id`, `concept_class`, `datatype`, `names[]`, `descriptions[]`, `extras`, `parent_concept_urls[]`, `hierarchy_meaning`, `note` |
...
validation.py                   rule replay, live probes, CLI report
